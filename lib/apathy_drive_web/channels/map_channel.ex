defmodule ApathyDriveWeb.MapChannel do
  ...
  def handle_info(:after_join, socket) do
    ...
    Room.world_map
    |> Repo.all
    |> Enum.map(fn %{id: area_id, level: level} ->
         Task.async fn ->
           {area_name, map} =
             area_id
             |> Room.area_map
             |> Repo.all
             |> Enum.reduce({nil, %{}}, fn %{id: id, area: area_name} = room, {_, map} ->
                  directions =
                    room.exits
                  ...
                  {area_name, Map.put(map, to_string(id), room)}
                end)

           if map_size(map) > 0, do: push socket, "update_map", %{area_name => %{level: level, rooms: map}}
         end
       end)
