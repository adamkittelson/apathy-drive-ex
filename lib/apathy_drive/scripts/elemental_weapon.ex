defmodule ApathyDrive.Scripts.ElementalWeapon do
  ...
  def execute(%Room{} = room, mobile_ref, item) do
    Room.update_mobile(room, mobile_ref, fn room, character ->
      ...
      Mobile.send_scroll(
        character,
        "<p><span class='blue'>You cast #{lore.name} weapon at #{item.name}!</span></p>"
      )

      Room.send_scroll(
        room,
        "<p><span class='blue'>#{character.name} casts #{lore.name} weapon at #{item.name}!</span></p>",
        [character]
      )
