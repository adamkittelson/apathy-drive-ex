defmodule ApathyDrive.Scripts.GreaterElementalWeapon do
  ...
  def execute(%Room{} = room, mobile_ref, item) do
    Room.update_mobile(room, mobile_ref, fn room, character ->
      ...
      effect = %{
        "WeaponDamage" => damage,
        "RemoveMessage" => "A greater #{lore.name} damage spell fades from your #{item.name}.",
      ...
      Mobile.send_scroll(
        character,
        "<p><span class='blue'>You cast greater #{lore.name} weapon at #{item.name}!</span></p>"
      )

      Room.send_scroll(
        room,
        "<p><span class='blue'>#{character.name} casts greater #{lore.name} weapon at #{item.name}!</span></p>",
        [character]
      )
