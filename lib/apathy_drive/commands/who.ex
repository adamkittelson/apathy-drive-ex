    ...
    |> Enum.each(fn
      %{name: name, game: game} ->
        if String.length(who_game) > 0 and String.starts_with?(String.downcase(game), who_game) do
          ...
          Mobile.send_scroll(
            character,
            "<p>        <span class='dark-green'>#{name} -</span> <span class='dark-magenta'>Outlander</span> <span class='dark-green'>of</span> <span class='dark-yellow'>#{game}</span>"
          )
        end
      ...
      %{name: name, title: title} ->
        if who_game == "" do
          ...
          Mobile.send_scroll(
            character,
            "<p><span class='dark-green'>#{name} - <span class='dark-magenta'>#{title}</span></p>"
          )
        end
    end)
