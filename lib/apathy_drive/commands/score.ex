defmodule ApathyDrive.Commands.Score do
  ...
  defp show_score(%Character{} = character, room) do
    ...
    Mobile.send_scroll(character, "<p><span class='dark-green'>Hits:</span> <span class='dark-cyan'>#{hits}</span><span class='dark-green'>Magical Dmg:</span> <span class='dark-cyan'>#{String.pad_trailing(to_string(trunc(score_data.magical_damage)), 7)}</span><span class='dark-green'>Tracking:</span> <span class='dark-cyan'>#{String.pad_leading(to_string(trunc(score_data.tracking)), 8)}</span> #{resistance(Enum.at(score_data.resistances, 6))} #{resistance(Enum.at(score_data.resistances, 7))}</p>")
