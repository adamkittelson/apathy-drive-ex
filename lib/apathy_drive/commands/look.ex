  ...
  def look_at_item(%Character{} = character, %Item{} = item, room) do
    ...
    case ApathyDrive.Commands.Wear.equip_item(character, item, false) do
      ...
      %{equipped: _, character: equipped} ->
        ...
        Mobile.send_scroll(character, "<p><span class='dark-green'>Strength:</span>  <span class='#{score_data.strength.color}'>#{String.ljust(to_string(trunc(score_data.strength.value)), 7)}</span> <span class='dark-green'>Agility:</span> <span class='#{score_data.agility.color}'>#{String.ljust(to_string(trunc(score_data.agility.value)), 11)}</span><span class='dark-green'>Spellcasting:</span> <span class='#{score_data.spellcasting.color}'>#{String.rjust(to_string(score_data.spellcasting.value), 4)}</span></p>")
        Mobile.send_scroll(character, "<p><span class='dark-green'>Intellect:</span> <span class='#{score_data.intellect.color}'>#{String.ljust(to_string(trunc(score_data.intellect.value)), 7)}</span> <span class='dark-green'>Health:</span>  <span class='#{score_data.health.color}'>#{String.ljust(to_string(trunc(score_data.health.value)), 11)}</span><span class='dark-green'>Crits:</span> <span class='#{score_data.crits.color}'>#{String.rjust(to_string(score_data.crits.value), 11)}</span></p>")
