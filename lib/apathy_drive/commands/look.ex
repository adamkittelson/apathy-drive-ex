  ...
  def look_at_item(%Character{} = character, %Item{} = item, room) do
    ...
    case ApathyDrive.Commands.Wear.equip_item(character, item, false) do
      ...
      %{equipped: _, character: equipped} ->
        ...
        Mobile.send_scroll(character, "<p><span class='dark-green'>Hits:</span> <span class='#{score_data.max_hp.color}'>#{hits}</span><span class='dark-green'>Magical Dmg:</span> <span class='#{score_data.magical_damage.color}'>#{String.pad_trailing(to_string(trunc(score_data.magical_damage.value)), 7)}</span><span class='dark-green'>Block:</span> <span class='#{score_data.block.color}'>#{String.pad_leading(to_string(trunc(score_data.block.value)), 11)}</span></p>")
