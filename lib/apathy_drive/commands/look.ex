defmodule ApathyDrive.Commands.Look do
  ...
  def look(%Room{} = room, %Character{} = character, arguments) when is_list(arguments) do
    cond do
      ...
      target = Room.find_mobile_in_room(room, character, Enum.join(arguments, " ")) ->
        look_at_mobile(room, target, character)
  ...
  def peek(%Room{} = room, %Character{} = character, room_id) do
    ...
    if mirror_exit do
      ...
      |> Enum.each(fn
        %Character{} = observer ->
          message =
            "#{Mobile.colored_name(character, room)} peeks in from #{
  ...
  def look_at_mobile(room, %{} = target, %Character{} = character) do
    if character != target,
      do:
        Mobile.send_scroll(
          target,
          "<p>#{Mobile.colored_name(character, room)} looks you over.</p>"
        )
  ...
  def look_mobiles(%Room{mobiles: mobiles} = room, character \\ nil) do
    mobiles_to_show =
      ...
      |> Enum.map(&Mobile.colored_name(&1, room))
