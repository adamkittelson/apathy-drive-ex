defmodule ApathyDrive.Commands.Leave do
    ...
    room =
      ...
      |> Enum.reduce(room, fn
        ...
        %Character{leader: ^ref, ref: char_ref} = char, updated_room ->
          Mobile.send_scroll(
            char,
            "<p><span class='blue'>#{character.name} has disbanded the party.</span></p>"
          )
  ...
  def execute(%Room{} = room, %Character{} = character, _arguments) do
    ...
    Mobile.send_scroll(
      character,
      "<p><span class='blue'>You are no longer following #{leader.name}.</span></p>"
    )

    room = put_in(room.mobiles[character.ref].leader, character.ref)
    Room.update_moblist(room)
    room
  end
end
