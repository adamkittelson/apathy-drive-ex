defmodule ApathyDriveWeb.RaceControllerTest do
  ...
  describe "create race" do
    ...
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post(conn, race_path(conn, :create), race: @create_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == race_path(conn, :show, id)

      conn = get(conn, race_path(conn, :show, id))
  ...
  describe "delete race" do
    ...
    test "deletes chosen race", %{conn: conn, race: race} do
      ...
      assert_error_sent(404, fn ->
        get(conn, race_path(conn, :show, race))
      end)
    end
  end
